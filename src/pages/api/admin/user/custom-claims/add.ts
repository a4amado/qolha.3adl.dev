...
export default nextConnect().use(isAdmin.middleware_auth_admin).post(async (req: NextApiRequest, res: NextApiResponse) => {
    try {
        ...
        const isEmail = user.email?.toLocaleLowerCase() === req.body.email.toLocaleLowerCase();
        ...
        return res.status(StatusCodes.OK).send({ message: StatusCodes.getStatusText(StatusCodes.UNPROCESSABLE_ENTITY) })
