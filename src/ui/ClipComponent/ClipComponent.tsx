...
export default function ClipComponent({ number, ar, username, clipId }: {
    number: number, ar: string, username: string, clipId: string
}) {
    ...
    return <div className={`flex p-1 justify-between ${number % 2 === 0 ? "bg-slate-200" : ""}`}>
        ...
        <ButtonGroup className="gap-2">
            ...
            {/** @ts-ignore */}
            {session.status === "authenticated" && ["owner", "admin"].includes(session.data?.user?.role || "") && <Button disabled={disabled[0]}
                onClick={async () => {
            ...
            {/** @ts-ignore */}
            {session.status === "authenticated" && ["owner", "admin"].includes(session.data?.user?.role || "") && <Button disabled={disabled[0]} onClick={async () => {
